...
import InvoiceComponents from "./components/InvoiceComponent";
...
export default function Home() {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      console.error("Erreur lors du changement des factures", error)
    }
  }
  useEffect(() => {
    fetchInvoices()
  ...
  const handeleCreateInvoice = async () => {
    try {
      if (email) {
    ...
    <Wrapper>
      <div className="flex flex-col space-y-4">
        ...
        <div className="grid md:grid-cols-3 gap-4">
           <div className="cursor-pointer border border-accent rounded-xl
        ...
        <dialog id="my_modal_3" className="modal">
          <div className="modal-box">
            ...
            <input
             type="text"
             placeholder="Nom de lea facture (max 60 caractéres"
              className="input input-bordered w-full my-4"
              value = {invoiceName}
              onChange = {(e) => setInvoiceName(e.target.value)}
               />
               {!isNameValid && <p className="mb-4 text-sm">Le nom ne peut pas dépasser 60 caractéres</p> } 
                   <button className="btn btn-accent"
                   disabled= {!isNameValid || invoiceName.length === 0}
                   onClick = {handeleCreateInvoice}
                   >
                    Créer
                    </button>          
